...
{
    //扩展函数不会自己加载，需要通过vkGetInstanceProcAddr，创建代理函数
...
void VulkanApp::initVulkan()
{
    createInstance();
    setupDebugMessenger();
    createSurface();//surface的创建会影响物理设备的选择
    //选择物理设备
    pickPhysicalDevice();
    //create logic device
    createLogicalDevice();
    ...
    createFramebuffers();//framebuffer 中需要包含深度attachment
    createTextureImage();
...
void VulkanApp::createInstance()
{
    ...
    VkInstanceCreateInfo createInfo{};//告诉Vulkan驱动要使用的全局扩展和验证层
    createInfo.sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO;
    createInfo.pApplicationInfo = &appInfo;

    //运行程序以确保您没有收到VK_ERROR_EXTENSION_NOT_PRESENT错误。验证层的可用性应隐含此扩展。
...
std::vector<const char*> VulkanApp::getRequiredExtensions()
{
    ...
    {
        //要在程序中设置一个回调来处理消息和相关的详细信息，我们必须使用VK_EXT_debug_utils扩展名通过回调创建调试信使。
...
{
    if (glfwCreateWindowSurface(m_instance, m_vulkan_window, nullptr, &m_surface) != VK_SUCCESS) {
...
VkExtent2D VulkanApp::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities)
{
    if (capabilities.currentExtent.width != UINT32_MAX) {
        ...
        std::cout << "actualextent w:" << actualExtent.width << "actualextent h:" << actualExtent.height;
        return actualExtent;
    }
}
...
{
    auto vs = readFile("./vert.spv");
    auto fs = readFile("./frag.spv");
    ...
    //请记住，交换链及其图像的大小可能与窗口的WIDTH和有所不同 HEIGHT。交换链图像稍后将用作帧缓冲区，因此我们应保持其大小。
    VkRect2D scissor{};
    scissor.offset = {0, 0};
    ...
    rasterizer.polygonMode = VK_POLYGON_MODE_FILL;//确定片断如何用于几何生成。
    rasterizer.lineWidth = 1.0f;
    rasterizer.cullMode = VK_CULL_MODE_BACK_BIT;
    rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;//设置指定面的绘制顶点的顺序为顺时针或逆时针
...
//    multisampling.alphaToOneEnable = VK_FALSE; //Optional
    ...
    //如果以上这两种blend模式都设置为vk_false，在这种情况下， 片断颜色将未经修改地写入帧缓冲区
    //uniform
...
void VulkanApp::createRenderPass()
{
    ...
    colorAttachmentRef.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;//颜色缓冲区
    //depthAttachment
...
void VulkanApp::createTextureImage()
{
    ...
    stbi_image_free(pixels);//create original pixels.
...
{
    VkCommandBuffer commandBuffer = beginSingleCommands();
    //确保在读取之前完成对缓冲区的写入---转移队列Family所有权
...
void VulkanApp::createDescriptorSets()
{
    ...
    //填充描述符
    for (size_t i = 0; i < m_swapChainImages.size(); i++) {
        VkDescriptorBufferInfo bufferInfo{}; // uniform
        bufferInfo.buffer = m_uniformBuffers[i];
        bufferInfo.offset = 0;
        bufferInfo.range = sizeof(UniformBufferObject);

        VkDescriptorImageInfo imageInfo{}; //texture
...
VkCommandBuffer VulkanApp::beginSingleCommands()
{
    ...
    beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;//我们只会使用命令缓冲区一次并等待从函数返回，直到操作完成执行
...
void VulkanApp::endSingleCommands(VkCommandBuffer commandBuffer)
{
